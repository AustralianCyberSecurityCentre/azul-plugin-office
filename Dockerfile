...
RUN uv pip uninstall --system azul-plugin-office
RUN uv pip install --system --no-deps --find-links /tmp/ azul-plugin-office==$(hatchling version)
...
RUN if [ "$GIT_BRANCH_NAME" = "refs/heads/dev" ] ; then \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-office' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$UV_INDEX_URL --system --upgrade --no-deps --prerelease allow '{}>=0.0.0-dev' \
    else \
    uv pip freeze | grep 'azul-.*==' | grep -v '^azul-plugin-office' | cut -d "=" -f 1 | xargs -I {} uv pip install --extra-index-url=$UV_INDEX_URL --system --upgrade --no-deps '{}>=0.0.0'\
    ;fi
...
USER root
COPY debian_tests.txt /tmp/src/
RUN apt-get update && \
    DEBIAN_FRONTEND=noninteractive \
    apt-get install -y --no-install-recommends \
    $(grep -vE "^\s*(#|$)" /tmp/src/debian_tests.txt | tr "\n" " ")
